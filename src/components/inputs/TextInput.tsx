...
const TextInput: React.FC<TextInputProps> = forwardRef(({ id, label, placeholder, value, onChange, isValid, errorMessage, style, minNumber, maxNumber, type = 'text' }, ref) => {
  ...
  return (
    ...
    <div className='mb-6' style={style} ref={ref}>
      ...
      <input type={type} value={value} min={minNumber} max={maxNumber} onChange={(ev) => onChange(ev.target.value)} placeholder={placeholder} id={id} className={`${inputClasses} text-sm rounded-lg block w-full p-2.5 dark:bg-neutral-700 dark:placeholder:text-neutral-300`} />
