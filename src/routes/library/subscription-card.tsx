...
const SubscriptionCard: React.FC<Props> = ({ sub }) => {
  let navigate = useNavigate()

  const item = useDoc(sub.shadow.resource_id)
  ...
  const isOwner =
    (user && item?.owner_id && user.id === item.owner_id) ||
    (!user && item?.owner_id === 'none');
                      ...
                      <DropdownMenuGroup>
                        ...
                        <DropdownMenuItem onClick={async () => {
                          ...
                          JSONToFile(
                            ...
                            `${item.shadow.name}-${versionData.local_id}`
                          )
                        }}>
                          Export
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => 
                          openModal('edit-string', ({ id }) => (
                            <EditString
                              id={id} title='Rename Subscription' data={item.shadow.name}
                              // onSave={(newName) => renameItem(item.type, item.local_id, newName)}
                              onSave={() => {}}
                            />)
                        )}>
                          Rename
            ...
            <DropdownMenuContent className='w-56' align='start'>
              <DropdownMenuGroup>
                <DropdownMenuItem onClick={async () => {
                  ...
                  //   },
                  //   `${item.shadow.name}-${latestVersion.local_id}`
                  // )
                }}>
                  Export
                </DropdownMenuItem>
