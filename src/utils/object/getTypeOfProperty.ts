...
export default function getTypeFromProperty(obj: { [key:string]: any } | null, path: string) {
  ...
  // check if a property is an object with a _type property.
  // if not, then we need to check parentProperty for the _type property and get the key that matches our property to get its type.
