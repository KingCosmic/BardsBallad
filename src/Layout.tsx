...
import { ScriptTypesProvider } from '@hooks/useScriptTypes'
...
const Layout: React.FC = () => {
  ...
  return (
    <ScriptTypesProvider>
      <div className='w-screen h-screen-dynamic transition-colors overflow-x-hidden fantasy-gradient fantasy-atmospheric text-fantasy-text'>
  ...
  <div onClick={closeMainSidebar} className={`${isOpen ? '' : '-translate-x-full'} bg-neutral-950 opacity-65 fixed top-0 left-0 z-40 w-screen sm:w-64 h-screen-dynamic sm:translate-x-0`} />
        ...
        <aside
          ...
          aria-label='Sidebar'
        >
          <div className='h-full flex flex-col justify-between px-3 py-4 overflow-y-auto' onClick={(e) => e.stopPropagation()}>
            ...
            <div>
              ...
              {/* Beta Message / Notice Message */}
              <div
                className='bg-gradient-to-br from-fantasy-accent/20 to-fantasy-accent-dark/10 border border-fantasy-accent/30 rounded-xl p-4 mt-5 text-sm leading-relaxed'
                role='alert'
              >
                <span className='bg-fantasy-accent text-fantasy-dark px-2 py-0.5 rounded text-sm font-bold mb-2 inline-block'>
                  Beta
                </span>
              ...
              <ul className='pt-4 mt-2 space-y-2 font-medium border-t border-neutral-200 dark:border-neutral-700'>
                ...
                {!auth.isLoggedIn && (
                  <li className='w-full'>
                    <button
                      className='w-full fantasy-accent-gradient text-fantasy-dark px-4 py-2 rounded-lg text-xs font-medium transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-fantasy-accent/40'
                      onClick={() => openModal('auth', ({ id }) => <AuthModal id={id} />)}
                    >
                      Login / Register
                    </button>
                  </li>
        ...
        {/* Content Outlet */}
        <div className='sm:ml-64 h-screen-dynamic p-8 lg:p-10'>
          <Outlet />
        </div>
